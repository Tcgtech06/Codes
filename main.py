...
class InvoiceApp(ct.CTk):
    ...
    def setup_sidebar(self):
        self.sidebar = ct.CTkFrame(self, width=280, corner_radius=0, fg_color="#f8fafc")
        self.sidebar.grid(row=0, column=0, sticky="nsew")

        # Add Application Logo at the top
        logo_path = "logo.png"
        if os.path.exists(logo_path):
            try:
                ...
                logo_photo = ct.CTkImage(light_image=logo_img, dark_image=logo_img, size=(120, 120))
                
                logo_label = ct.CTkLabel(self.sidebar, image=logo_photo, text="")
                ...
                ct.CTkLabel(self.sidebar, text="INVOICE SYSTEM", 
                           font=ct.CTkFont(size=16, weight="bold")).pack(pady=(20, 10))
        else:
            ct.CTkLabel(self.sidebar, text="INVOICE SYSTEM", 
                       font=ct.CTkFont(size=16, weight="bold")).pack(pady=(20, 10))

        # User/Business Switcher Title
        ct.CTkLabel(self.sidebar, text="SELECT BUSINESS",
                    font=ct.CTkFont(size=14, weight="bold"), text_color="#64748b").pack(pady=(30, 10))

        # Business Switcher Buttons (With Icons)
        self.biz_btn_frame = ct.CTkFrame(self.sidebar, fg_color="transparent")
        self.biz_btn_frame.pack(fill="x", padx=10)

        self.biz_buttons = {}

        # Create a button for each profile
        for biz_id, data in self.profiles.items():
            # Use emoji icon + Business Name (not biz_id)
        ...
        ct.CTkFrame(self.sidebar, height=2, fg_color="#e2e8f0").pack(fill="x", pady=20, padx=20)

        # Style Indicator
        self.style_label = ct.CTkLabel(self.sidebar, text="Current Style: Modern",
                                       font=ct.CTkFont(size=12, slant="italic"))
        self.style_label.pack(pady=5)

        # History Section
        ct.CTkLabel(self.sidebar, text="RECENT INVOICES",
                    font=ct.CTkFont(size=14, weight="bold"), text_color="#64748b").pack(pady=10)

        self.history_list = ct.CTkScrollableFrame(self.sidebar, fg_color="transparent")
        self.history_list.pack(padx=10, pady=10, fill="both", expand=True)
        self.refresh_history_ui()

    def setup_main_content(self):
        self.main_frame = ct.CTkScrollableFrame(self, corner_radius=0, fg_color="white")
        self.main_frame.grid(row=0, column=1, sticky="nsew")

        # --- Top Bar: Business Settings ---
        self.settings_frame = ct.CTkFrame(self.main_frame, fg_color="#f1f5f9", height=100)
        self.settings_frame.pack(fill="x", padx=40, pady=20)

        # Logo Display
        self.logo_preview = ct.CTkLabel(self.settings_frame, text="No Logo", width=80, height=80, fg_color="#e2e8f0",
                                        corner_radius=5)
        self.logo_preview.pack(side="left", padx=20, pady=10)

        # Business Info Inputs
        info_frame = ct.CTkFrame(self.settings_frame, fg_color="transparent")
        info_frame.pack(side="left", fill="y", pady=10)

        self.biz_name_entry = ct.CTkEntry(info_frame, width=300, font=ct.CTkFont(size=20, weight="bold"),
                                          placeholder_text="Business Name")
        self.biz_name_entry.pack(anchor="w", pady=(0, 5))

        self.biz_addr_entry = ct.CTkEntry(info_frame, width=300, placeholder_text="Business Address")
        self.biz_addr_entry.pack(anchor="w", pady=(0, 5))

        self.biz_email_entry = ct.CTkEntry(info_frame, width=300, placeholder_text="Business Email")
        self.biz_email_entry.pack(anchor="w", pady=(0, 5))

        self.biz_phone_entry = ct.CTkEntry(info_frame, width=300, placeholder_text="Business Phone")
        self.biz_phone_entry.pack(anchor="w")

        # Save & Upload Buttons
        btn_frame = ct.CTkFrame(self.settings_frame, fg_color="transparent")
        btn_frame.pack(side="right", padx=20)

        ct.CTkButton(btn_frame, text="Upload Logo", command=self.upload_logo, width=120, fg_color="#475569").pack(
            pady=5)
        self.save_biz_btn = ct.CTkButton(btn_frame, text="Save Biz Info", command=self.save_current_profile_info,
                                         width=120)
        self.save_biz_btn.pack(pady=5)

        # --- Divider ---
        ct.CTkFrame(self.main_frame, height=2, fg_color="#e2e8f0").pack(fill="x", padx=40, pady=10)

        # --- Invoice Form ---
        ct.CTkLabel(self.main_frame, text="CLIENT DETAILS", font=ct.CTkFont(size=14, weight="bold"),
                    text_color="#64748b").pack(anchor="w", padx=40)

        client_frame = ct.CTkFrame(self.main_frame, fg_color="transparent")
        client_frame.pack(fill="x", padx=30, pady=10)
        ...
        # Due Date with Pending Checkbox
        ...
        lbl = ct.CTkLabel(due_date_container, text="Due Date", font=ct.CTkFont(size=12, weight="bold"))
        lbl.pack(anchor="w")
        
        due_date_inner = ct.CTkFrame(due_date_container, fg_color="transparent")
        due_date_inner.pack(fill="x")
        
        self.due_date = ct.CTkEntry(due_date_inner, height=35)
        self.due_date.pack(side="left", fill="x", expand=True, padx=(0, 5))
        ...
        # --- Items Section ---
        self.item_frame_container = ct.CTkFrame(self.main_frame, fg_color="#f8fafc", corner_radius=10)
        self.item_frame_container.pack(fill="x", padx=40, pady=20)

        entry_grid = ct.CTkFrame(self.item_frame_container, fg_color="transparent")
        entry_grid.pack(fill="x", padx=15, pady=15)

        self.item_desc = ct.CTkEntry(entry_grid, placeholder_text="Description", width=400, height=40)
        self.item_desc.pack(side="left", padx=5)

        self.item_price = ct.CTkEntry(entry_grid, placeholder_text="Amount", width=150, height=40)
        self.item_price.pack(side="left", padx=5)

        self.add_btn = ct.CTkButton(entry_grid, text="+", width=50, height=40, command=self.add_item)
        self.add_btn.pack(side="left", padx=5)

        # --- Table & Tax ---
        self.table_frame = ct.CTkFrame(self.main_frame, fg_color="white", border_width=1, border_color="#e2e8f0")
        self.table_frame.pack(fill="x", padx=40, pady=10)

        # Tax Calculation Row
        tax_frame = ct.CTkFrame(self.main_frame, fg_color="transparent")
        tax_frame.pack(fill="x", padx=40, pady=5)

        ct.CTkLabel(tax_frame, text="Tax Rate (%):", font=ct.CTkFont(weight="bold")).pack(side="left")
        self.tax_entry = ct.CTkEntry(tax_frame, width=80, placeholder_text="0")
        self.tax_entry.pack(side="left", padx=10)
        self.tax_entry.insert(0, "0")

        # Bind key release to recalc total when tax changes
        self.tax_entry.bind("<KeyRelease>", lambda e: self.refresh_table())

        ct.CTkButton(tax_frame, text="Update Total", width=100, height=30,
                     fg_color="#64748b", command=self.refresh_table).pack(side="left")
        ...
        self.btn_pdf = ct.CTkButton(action_frame, text="Generate PDF", height=50, command=lambda: self.generate("pdf"))
        self.btn_pdf.pack(side="right", padx=10)

        self.btn_word = ct.CTkButton(action_frame, text="Generate Word", height=50,
                                     command=lambda: self.generate("word"), fg_color="#3b82f6")
        self.btn_word.pack(side="right", padx=10)

        ct.CTkButton(action_frame, text="Clear Form", height=50, fg_color="#64748b", command=self.clear_form).pack(
            side="right", padx=10)
    ...
    def create_input(self, master, placeholder):
        ...
        lbl = ct.CTkLabel(container, text=placeholder, font=ct.CTkFont(size=12, weight="bold"))
        lbl.pack(anchor="w")
        entry = ct.CTkEntry(container, height=35)
        entry.pack(fill="x")
        return entry
    ...
    def refresh_theme(self):
        # 1. Update Sidebar Buttons visual state
        for bid, btn in self.biz_buttons.items():
            if bid == self.current_biz_id:
                btn.configure(fg_color=self.current_theme, text_color="white", border_width=0)
            else:
                btn.configure(fg_color="white", text_color="black", border_width=1)
    ...
    def refresh_table(self):
        ...
        # Header
        h = ct.CTkFrame(self.table_frame, fg_color="#f1f5f9", height=30)
        h.pack(fill="x")
        ct.CTkLabel(h, text="Item Description", font=ct.CTkFont(weight="bold")).pack(side="left", padx=10)
        ct.CTkLabel(h, text="Price (₹)", font=ct.CTkFont(weight="bold")).pack(side="right", padx=20)

        subtotal = 0.0
        for item in self.items:
            row = ct.CTkFrame(self.table_frame, fg_color="transparent")
            row.pack(fill="x", pady=2)
            ct.CTkLabel(row, text=item['desc']).pack(side="left", padx=10)
            ct.CTkLabel(row, text=f"₹{item['price']:.2f}").pack(side="right", padx=20)
            subtotal += item['price']
        ...
        # Summary Frame
        summary_frame = ct.CTkFrame(self.table_frame, fg_color="transparent")
        summary_frame.pack(fill="x", pady=10, padx=20)

        ct.CTkLabel(summary_frame, text=f"Subtotal: ₹{subtotal:.2f}").pack(anchor="e")
        ct.CTkLabel(summary_frame, text=f"Tax ({tax_rate}%): ₹{tax_amt:.2f}").pack(anchor="e")
        ct.CTkLabel(summary_frame, text=f"GRAND TOTAL: ₹{grand_total:.2f}",
                    font=ct.CTkFont(size=18, weight="bold"), text_color=self.current_theme).pack(anchor="e")
    ...
    def refresh_history_ui(self):
        ...
        if not self.history:
            ct.CTkLabel(self.history_list, text="No recent invoices", 
                       text_color="#999").pack(pady=10)
            return
        
        # Show only last 5 invoices
        for h in reversed(self.history[-5:]):
            # Create frame for each invoice item
            item_frame = ct.CTkFrame(self.history_list, fg_color="white", height=40)
            item_frame.pack(fill="x", pady=3, padx=5)
            item_frame.pack_propagate(False)  # Prevent frame from shrinking
            
            # Invoice button
            btn = ct.CTkButton(
                item_frame,
                text=f"#{h['id']} - {h['client']}",
            ...
            btn.pack(side="left", fill="both", expand=True, padx=(5, 2))
