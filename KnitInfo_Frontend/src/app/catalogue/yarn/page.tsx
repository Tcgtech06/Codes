...
export default function YarnPage() {
  ...
  const parseExpiry = (expiresAt: unknown): Date | null => {
    if (!expiresAt) return null;
    if (typeof expiresAt === 'string') return new Date(expiresAt);
    if ((expiresAt as any)?.toDate) return (expiresAt as any).toDate();
    return null;
  };
  ...
  const filteredCompanies = companies.filter(company => {
    const matchesSearch = company.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
                         ...
                         normalizeProducts(company.products).some((p: string) => p.toLowerCase().includes(searchQuery.toLowerCase()));
  ...
  const sortedCompanies = [...filteredCompanies].sort((a, b) => {
    const aPriority = priorities.find(p => 
      ...
      (() => {
        const expiry = parseExpiry(p.expiresAt);
